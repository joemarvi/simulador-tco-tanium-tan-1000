...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Check attempts
    currentAttempt = checkAttempts(currentAttempt, MAX_ATTEMPTS, lastAttemptTime, BLOCK_TIME, startBtn);
        ...
        if (studyMode) {
            ...
            options.forEach(opt => {
                ...
                if (q.type === "multi_select") {
                    ...
                    if (opt.checked && !q.correct.includes(value)) label.classList.add("wrong");
                } else {
                    if (value === q.correct) label.classList.add("correct");
                    if (opt.checked && value !== q.correct) label.classList.add("wrong");
                }
            });

            showExplanation(questions, explanationPanel, explanationContent, index);
        }
    ...
    if (studyModeBtn) {
        studyModeBtn.addEventListener("click", () => {
            studyMode = toggleStudyMode(studyMode, studyModeStatus, studyModeBtn, explanationPanel, questions, showExplanation);
        });
    }
    ...
    if (finishBtn) {
        finishBtn.addEventListener("click", () => {
            ...
            if (currentAttemptEl) currentAttemptEl.textContent = currentAttempt;
            if (remainingAttemptsEl) remainingAttemptsEl.textContent = MAX_ATTEMPTS - currentAttempt;
            ...
            questions.forEach((q, i) => {
                ...
                let correct = (q.type === "multi_select") ? JSON.stringify([...selected].sort()) === JSON.stringify([...q.correct].sort()) : selected[0] === q.correct;
            ...
            if (resultContent) {
                resultContent.innerHTML = `<h2>Resultado</h2>Pontuação: ${score} / ${questions.length}<br>Percentual: ${percent}%<br>Resultado: <b>${percent >= 70 ? "✅ APROVADO" : "❌ REPROVADO"}</b>`;
            }
...
if (toggleIntro && introContent) {
    toggleIntro.addEventListener("click", () => {
        if (introContent.style.display === "none") {
            ...
            toggleIntro.innerHTML = '<i class="fa-solid fa-chevron-up"></i> Ocultar';
        } else {
            introContent.style.display = "none";
            toggleIntro.innerHTML = '<i class="fa-solid fa-chevron-down"></i> Mostrar';
        }
    });
}